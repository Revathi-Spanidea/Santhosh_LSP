...
#include <linux/kdev_t.h> //Defines the dev_t data type used for device numbers.
...
#include <linux/device.h> //Header for the Linux device model, providing structures and functions for creating device classes and managing devices.
#include<linux/slab.h>    //kmalloc() Includes memory allocation functions like kmalloc and kfree. Used for dynamic memory allocation.
...
#include <linux/err.h>   //Defines error-related macros, such as IS_ERR and PTR_ERR, for handling error returns.
...
static int __init etx_driver_init(void)
{
        ...
        if(IS_ERR(dev_class = class_create("etx_class"))){    //owner and name od module.used to create a struct class pointer that can then be used in calls to class_device_create.
            pr_err("Cannot create the struct class\n");                   //The return value can be checked using IS_ERR() macro.
            goto r_class;
        }
 
        //Creates a device file in /dev associated with the class.
        if(IS_ERR(device_create(dev_class,NULL,dev,NULL,"etx_device"))){  //Class- pointer to the struct class that this device should be registere,
