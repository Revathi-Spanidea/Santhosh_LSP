//:Build a parallel processing system using shared memory for inter-process communication.
...
{
    int shmid;
    key_t key = ftok("shared_memory_key", 65); // Generate a key for shared memory The ftok() function returns a key based on path d that is usable in subsequent calls to msgget(), semget(), and shmget()
    ...
    sharedData->data = 10;  // Example initial data
